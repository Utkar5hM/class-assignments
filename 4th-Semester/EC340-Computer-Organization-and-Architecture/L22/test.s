...
rgb2gray:   slti    $t3, $a2, 10         # if i == 10 goto done
        ...
        lw      $t5, 0($t4)         # t5= memory[address_1]  /// t5 = rgb[i]
        andi    $t6, $t5, 0xFF      # t6 = rgb[i][7:0] = blue value  // last 7 bits by using AND 
        srl     $t5, $t5, 8         # right shift t5 by a byte so GREEN comes at lowest significant byte.
        andi    $t7, $t5, 0xFF      # t7 = rgb[i][7:0] = green value  // last 7 bits by using AND 
        add     $t6, $t7, $t6       # t6 = t7 + t6 // t6 = blue + green
        srl     $t5, $t5, 8         # right shift t5 by a byte so RED comes at lowest significant byte.
        andi    $t7, $t5, 0xFF      # t7 = t5[i][7:0] = green value  // last 7 bits by using AND  
        add     $t6, $t7, $t6       # t6 = t7 + t6 // t6 = blue + green + red
        divu    $t6, $t6, $s3       # t6 = t6 / t8 = (blue + green + red) /3
        add     $t4, $s1, $t2       # address_2 = base_address + offset;
        sw      $t6, 0($t4)         #  memory[address_1]= t6  /// gray[i] = t6 = (r+g+b)/3 
        addi    $a2, $a2, 1         # i++
        j       rgb2gray
...
printl: slti    $t3, $a3, 10         # if i == 10 goto done
        ...
        add     $t2, $s0, $t6       # rgb_address = base_rgb_address + offset;
        lw      $t4, 0($t2)        # t4 = rgb[i] 
        li      $v0, 4
        ...
        syscall
        add     $t2, $s1, $t6       # gray_address = base_gray_address + offset;
